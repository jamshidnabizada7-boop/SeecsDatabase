...
export async function PUT(req: Request, ctx: Ctx) {
  ...
  try {
    const item = await db.sector.update({ where: { id }, data: { name: String(name).trim() } })
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  if (!s) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const { id } = await ctx.params
  const inUse = await db.company.count({ where: { sectorId: id } })
  if (inUse > 0) return NextResponse.json({ error: `Cannot delete: ${inUse} company(ies) use this sector` }, { status: 409 })
  try {
